...
def neigh_match(ename_embed, train_links_array, ent_neigh_dict_old, max_neighbors_num, ent_pad_id, rel_pad_id, myconfig):
    ...
    for start_pos in range(0, len(train_links_array), batch_size):  # len(ent_pairs)=750000
        ...
        min_score, min_index = alignment.torch_sim_min_batch(e1_neigh_emb, e2_neigh_emb, top_num=1, metric='cosine') #is_detach=False
        if min_index.is_cuda:
            min_score = min_score.detach().cpu().numpy()
            min_index = min_index.detach().cpu().numpy()
        else:
            min_score = min_score.detach().numpy()
            min_index = min_index.detach().numpy()

        min_score = min_score.squeeze(-1)  # [B,neigh,1] -> [B,neigh] #get max value.
            ...
            if len(e2_array) != len(set(e2_array.tolist())): # Many to one
                for i in range(1, len(e2_array)):
                    ...
                    if e2_array[i] in e2_array[0: i]:
                        ...
                        if score_array[index] > score_array[i]:
                            e2_array[i] = ent_pad_id
                        else:
                            e2_array[index] = ent_pad_id
            aa = np.vstack((e1_array, e2_array, r1_array, r2_array, score_array)).T
            batch_match[e] = aa
...
def rel_match(left_neigh_match, ent_pad_id, myconfig):
    ...
    for neigh_ll in left_neigh_match.tolist():
        for (tail_i, tail_j, rel_i, rel_j, ent_ij_sim) in neigh_ll:
            ...
            if ent_ij_sim > 1: # Exclude low similarity
                ent_ij_sim5_num += 1
                continue

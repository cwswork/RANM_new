...
class align_union_set_100K():
    def __init__(self, config):
        super(align_union_set_100K, self).__init__()
        self.config = config
    ...
    def model_train(self, beg_epochs=0):
        ...
        for epochs_i in range(beg_epochs, self.config.train_epochs):  # epochs=1001
            ...
            if epochs_i >= self.config.start_valid and epochs_i % self.config.eval_freq == 0:
                break_re = self.runTest(epochs_i, isSave=False)
    ...
    def reRunTest(self, better_epochs_i, epochs_name, isSave=False):
        ...
        self.runTestFile(model_savefile, better_epochs_i, isSave=isSave, is_cuda=self.config.is_cuda)


    def runTestFile(self, model_savefile, better_epochs_i, isSave=False, is_cuda=False):
        ...
        # 运行测试
        self.runTest(better_epochs_i, isSave=isSave)

...
def gen_candidate(ename_embed, train_links_array, valid_links_array, test_links_array, candidate_num,
    ...
    #ent_candidates = test_candidates
...
def gen_candidate_double(ent_emb, ents1, ents2, candidate_num, metric='L1', is_cuda=True):
    ...
    # kg1
    max_index = alignment.torch_sim_max_topk(emb1, emb2, top_num=candidate_num, metric=metric)
    # return the Cosine similarity matrix of embedding1 (emb1) and embedding2 (emb2)
    ent2candidates = dict()  # {e1: e2_list}
    ents2array2 = np.array(ents2)
    for i in range(len(max_index)):
        e1 = ents1[i]
        e2_list = ents2array2[max_index[i]].tolist()
        ent2candidates[e1] = e2_list
    # kg2
    max_index = alignment.torch_sim_max_topk(emb2, emb1, top_num=candidate_num, metric=metric)
    ents2array1 = np.array(ents1)
    for i in range(len(max_index)):
        e2 = ents2[i]
        e1_list = ents2array1[max_index[i]].tolist()
...
def candidate_partition(ent_emb, ents1, ents2, candidate_num_1, metric='L1', is_cuda=True):
    ...
    max_index = alignment.torch_sim_max_topk(emb1, emb2, top_num=candidate_num_1, metric=metric)
    ent2candidates = dict()  # {e1: e2_list}
    ents2array = np.array(ents2)
    for i in range(len(max_index)):
        e1 = ents1[i]
        e2_list = ents2array[max_index[i]].tolist()
        ent2candidates[e1] = e2_list
            ...
            if (e1_old, e2_old) in temp_rel_match_dict.keys():
                ...
                if r_union != oldent_rset[e1_old]:
                    if r_union not in newent_reset[e1_old].values():
                        ...
                        for neweid, rset in newent_reset[e1_old].items(): # new id
                            if r_union == rset:
                                e1_new = neweid
                ...
                if r_union != oldent_rset[e2_old]:
                    if r_union not in newent_reset[e2_old].values():
                        ...
                        temp_ent_old2list_dict[e2_old].append(e2_new)
                        kg2_ent_new.append(newnet_id) # new ID
                        newnet_id += 1
                    else:
